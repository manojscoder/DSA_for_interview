...
class segmentTree:
    ...
    def build(self, index, low, high, arr):
        ...
        self.segment[index] = self.segment[2 * index + 1] + self.segment[2 * index + 2]
...
DS = segmentTree(arr)
print("Total sum in range 3 and 8 is:", DS.query(0, 0, DS.size - 1, 3, 8))
print("Total sum in range 2 and 5 is:", DS.query(0, 0, DS.size - 1, 2, 5))
print("Total sum in range 1 and 9 is:", DS.query(0, 0, DS.size - 1, 1, 9))
